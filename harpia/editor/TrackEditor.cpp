...
void TrackEditor::ensureStripCache(const QRect &src)
{
	...
	if (!thumbs_.isEmpty() && duration_ > 0) {
		...
		// One tile per DISTINCT frame, so zooming in spaces the thumbnails out
		// instead of drawing the same one five times (see Filmstrip.hpp).
		const qint64 fromMs = xToMs(src.x());
		const qint64 spanMs = std::max<qint64>(1, xToMs(src.x() + local.width()) - fromMs);
